...
- Name: error-handling.spec.ts >> Error Handling & Network Interruptions >> should handle Supabase 500 API errors gracefully
- Location: e2e\error-handling.spec.ts:25:7
...
Locator: locator('text=Internal Server Error').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=Internal Server Error').first()
...
```ts
  ...
  18 |     const errorMsg = page.locator('text=/network|offline|failed to fetch/i').first();
